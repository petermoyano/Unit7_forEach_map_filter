...
function vowelCount(str) {
    let casInsStr = str.toLowerCase();
    let res = {};
    let vows = "aeiou";
    let strInArr = casInsStr.split("");
    strInArr.forEach(callback);
...
function removeVowels(str) {
    let loCamStr = str.toLowerCase();
    let arrStr = loCamStr.split("");
    let vowels = "aeiou"
    let noVowlesArr = arrStr.filter(function (char) {
        return (!vowels.includes(char))

    });
    return ((noVowlesArr.toString()).replace(/,/g, ""));
}
...
function doubleOddNumbers(arr) {
    let onlyOdds = arr.filter(function (val) {
        return (!(val % 2 === 0))
    });
    console.log(onlyOdds)
    let doubleArr = onlyOdds.map(function (val) {
        return val * 2;
    })
    return (doubleArr);
